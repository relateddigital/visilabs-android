...
public class Visilabs implements VisilabsURLConnectionCallbackInterface {
    ...
    private static int mLogLevel = VisilabsConfig.LOG_LEVEL_ERROR;
                ...
                try {
                    ...
                    if (array != null) {
                        ...
                        if (notificationToShow == null && type != null) {
                            for (int i = 0; i < notifications.size(); i++) {
                                if (notifications.get(i) != null && notifications.get(i).getType().toString() == type) {
                                    notificationToShow = notifications.get(i);
                                    break;
                                }
                            }
...
//        if(this.mGeofenceEnabled && !StringUtils.isNullOrWhiteSpace(this.mGeofenceURL)){
//            startGpsManager();
//
//            Timer timer = new Timer("startGps", false);
...
//        VisilabsGeofenceRequest request = (VisilabsGeofenceRequest) new VisilabsGeofenceRequest(this._context);
...
//        }
    ...
    public List<VisilabsGeoFenceEntity> getActiveGeofences() {
        List<VisilabsGeoFenceEntity> geoFences = new ArrayList<>();
