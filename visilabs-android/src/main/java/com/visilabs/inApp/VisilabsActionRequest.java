...
public class VisilabsActionRequest extends VisilabsRemote {
    ...
    public void executeAsyncNpsWithNumbersAction (final VisilabsCallback pCallback) throws Exception {
        if (Build.VERSION.SDK_INT < VisilabsConstant.UI_FEATURES_MIN_API) {
            Log.e("Visilabs", "Visilabs SDK requires min API level 21!");
            return;
        }
        ...
        try {
            Call<ResponseBody> call = mVisilabsSApiInterface.getNpsWithNumbersResponse(headers, queryParameters);
            call.enqueue(new Callback<ResponseBody>() {
                @Override
                public void onResponse(Call<ResponseBody> call, Response<ResponseBody> response) {
                    if(response.isSuccessful()) {
                        ...
                        try {
                            ...
                            if (!rawJsonResponse.equals("")) {
                                JSONArray mainArray = new JSONArray(rawJsonResponse);
                                if(mainArray != null && mainArray.length() > 0) {
                                    Log.i(LOG_TAG, "Success Request : " + response.raw().request().url().toString());
                                    VisilabsResponse visilabsResponse = new VisilabsResponse(new JSONArray(rawJsonResponse), null, null, null, null);
                                    pCallback.success(visilabsResponse);
                                } else {
                                    Log.e(LOG_TAG, "Empty response for the request : " + response.raw().request().url().toString());
                                    VisilabsResponse visilabsResponse = new VisilabsResponse(null, null, "empty string", null, "empty string");
